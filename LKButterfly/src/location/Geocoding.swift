...
public extension ViewControllerAccess {
    ...
    //--- ViewControllerAccess.geocode(String, (List<GeoAddress>)->Unit)
...
private func translate(mark: CLPlacemark) -> GeoAddress {
    return GeoAddress(coordinate: mark.location?.coordinate.toButterfly(), name: mark.name, street: mark.street, subLocality: mark.subLocality, locality: mark.locality, subAdminArea: mark.subAdministrativeArea, adminArea: mark.administrativeArea, countryName: mark.country, postalCode: mark.postalCode)
}

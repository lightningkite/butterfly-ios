...
open class MonthCVD : CustomViewDelegate {
    override public init() {
        ...
        self.rectForReuse = CGRect()
        self.rectForReuseB = CGRect()
        self.isTap = false
        self.dragStartY = 0
        super.init()
    ...
    open func drawMonth(canvas: CGContext, xOffset: CGFloat, width: CGFloat, month: DateAlone, displayMetrics: DisplayMetrics) -> Void {
        for day in ((1...7)){
            let col = day - 1
    ...
    open func drawLabel(canvas: CGContext, dayOfWeek: Int, displayMetrics: DisplayMetrics, outer: CGRect, inner: CGRect) -> Void {
        CalendarDrawing.INSTANCE.label(canvas: canvas, dayOfWeek: dayOfWeek, inner: inner, paint: self.labelPaint)
    }
    
    open func drawDay(canvas: CGContext, showingMonth: DateAlone, day: DateAlone, displayMetrics: DisplayMetrics, outer: CGRect, inner: CGRect) -> Void {
...
public class CalendarDrawing {
    ...
    public func day(canvas: CGContext, month: DateAlone, date: DateAlone, inner: CGRect, paint: Paint) -> Void {
    ...
    public func label(canvas: CGContext, dayOfWeek: Int, inner: CGRect, paint: Paint) -> Void {
    ...
    public func dayBackground(canvas: CGContext, inner: CGRect, paint: Paint) -> Void {
        canvas.drawCircle(inner.centerX(), inner.centerY(), Swift.min(inner.width() / 2, inner.height() / 2), paint)
    }
    
    public func dayBackgroundStart(canvas: CGContext, inner: CGRect, outer: CGRect, paint: Paint) -> Void {
        canvas.drawCircle(inner.centerX(), inner.centerY(), Swift.min(inner.width() / 2, inner.height() / 2), paint)
        canvas.drawRect(outer.centerX(), inner.top, outer.right, inner.bottom, paint)
    }
    
    public func dayBackgroundMid(canvas: CGContext, inner: CGRect, outer: CGRect, paint: Paint) -> Void {
        canvas.drawRect(outer.left, inner.top, outer.right, inner.bottom, paint)
    }
    
    public func dayBackgroundEnd(canvas: CGContext, inner: CGRect, outer: CGRect, paint: Paint) -> Void {
        canvas.drawCircle(inner.centerX(), inner.centerY(), Swift.min(inner.width() / 2, inner.height() / 2), paint)

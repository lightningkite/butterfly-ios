...
//--- Spinner.bind(ObservableProperty<List<T>>, MutableObservableProperty<T>, (ObservableProperty<T>)->UIView)
public extension Dropdown {
    func bind<T: Equatable>(_ options: ObservableProperty<Array<T>>, _ selected: MutableObservableProperty<T>, _ makeView: @escaping (ObservableProperty<T>) -> UIView) -> Void {

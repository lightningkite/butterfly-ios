...
private class LinearLayoutBoundSubview<T> {
    ...
    public init(view: UIView, property: StandardObservableProperty<T>) {
        self.view = view
...
public extension LinearLayout {
    func bindHorizontal<T>(data: ObservableProperty<Array<T>>, defaultValue: T, makeView: @escaping  (ObservableProperty<T>) -> UIView) -> Void {
        var existingViews: Array<LinearLayoutBoundSubview<T>> = []
        data.subscribeBy(onNext: { (value) -> Void in 
                //Fix view count
                let excessViews = existingViews.count - value.count
